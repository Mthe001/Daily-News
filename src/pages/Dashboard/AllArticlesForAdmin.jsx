...
const AllArticlesForAdmin = () => {
    ...
    return (
        <div>
            <Helmet>
                <title>All Articles ! News 240</title>
            </Helmet>
            {isLoading ? (
                <div className="flex items-center justify-center gap-3 h-screen">
                    <Loader /> <h1 className="text-4xl text-gray-800">Loading</h1>
                </div>
            ) : articles.length > 0 ? (
                <div className="overflow-x-auto w-11/12 mx-auto">
                    <table className="table text-gray-800"> {/* Ensuring text is gray on white */}
                        {/* head */}
                        <thead>
                            <tr>
                                <th>#</th>
                        ...
                        <tbody>
                            ...
                            {articles?.map((article, idx) => (
                                <tr key={article._id}>
                                    ...
                                    <td className="font-semibold text-gray-900"> {/* Adjusted text color */}
                                        <h1>{article.title}</h1>
                                        {article.isPremium && <h2 className="p-2 badge bg-orange-400">Premium</h2>}
                                    </td>
                                    <td className="capitalize text-nowrap text-gray-700">{article.authorName}
                                        {article.postedDate && (
                                            <p
                                                data-tooltip-id="my-tooltip"
                                                data-tooltip-content={moment(article.postedDate).format('lll')}
                                                className="p-0 text-sm text-gray-600"
                                            >
                                                Date: {moment(article.postedDate).format('ll')}
                                            </p>
                                        )}
                                    </td>
                                    <th className="text-gray-700">
                                        {article.authorEmail}
                                        <p className="font-normal text-gray-600">Publisher: {article.publisher}</p>
                                    </th>
                                    <td className="capitalize text-gray-700">{article.status}</td>
                                    <td className="flex items-center justify-start gap-2">
                                        ...
                                        <Button
                                            disabled={article.status === "declined" || article.isPremium}
                ...
                <h2 className="text-3xl text-gray-800">Data not available</h2> 
        )}
