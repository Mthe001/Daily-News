...
export default function UpdateUserProfile({ isOpen, close }) {
    ...
    const handleUpdateUser = async () => {
        ...
        try {
            const updatedUser = { name, image: imageUrl, email: user?.email };
            await updateUser(name, imageUrl);
            const { data } = await axiosSecure.patch(`/update-user`, updatedUser);

            if (data.modifiedCount > 0) {
                // Update the user state directly
                const newUser = { ...user, name, photoURL: imageUrl };
                setUser(newUser); // Update the global user state

                // Navigate to the desired page after successful update
                navigate('/');  // Change '/profile' to the page you want to navigate to
        ...
        <Dialog open={isOpen} as="div" className="relative z-10 focus:outline-none" onClose={close}>
            <div className="fixed inset-0 z-10 w-screen overflow-y-auto">
                <div className="flex min-h-full items-center justify-center p-4">
                    <DialogPanel
                        className="w-full max-w-md rounded-xl bg-black/70 p-6 backdrop-blur-2xl"
                    >
                        ...
                        <div className="mt-4">
                            <div className="flex items-center justify-end gap-5">
                                <Button
                                    className="inline-flex items-center gap-2 rounded-md bg-gray-700 py-1.5 px-3 text-sm/6 font-semibold text-white shadow-inner shadow-white/10 focus:outline-none data-[hover]:bg-gray-600"
                                    onClick={close}
                                >
                                    Cancel
                                </Button>
